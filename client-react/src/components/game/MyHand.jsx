...
export default function MyHand({ hand = [], playable = [], myTurn, onPlayCard }) {
  ...
  return (
    <div className="w-full flex flex-col items-center pointer-events-none">
      <div className="text-[10px] uppercase tracking-[0.3em] text-ink-faint mb-1.5 pointer-events-auto">
        Your hand · {hand.length}
      </div>
      <div className="relative h-32 sm:h-36 w-full max-w-3xl pointer-events-auto">
            ...
            {hand.map((card, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="-ml-4 first:ml-0 hover:z-10"
                  style={{ zIndex: i }}
                >
                  <Card
                    card={card}
